...
@Component({
  ...
  styles: [`
    .msg-layout { display:flex; height:calc(100vh - 120px); background:white; border-radius:16px; overflow:hidden; box-shadow:var(--shadow); }
    ...
    @media (max-width:640px) {
      ...
      .conv-list { width:100%; border-right:none; border-bottom:1.5px solid #f0ebff; max-height:220px; }
    }
  `],
  template: `
        ...
        <div class="chat-area">
          @if (!selectedCompanyId()) {
            ...
            <div class="chat-header">
              <div style="width:36px;height:36px;border-radius:50%;background:var(--gradient);display:flex;align-items:center;justify-content:center;color:white;font-weight:800;font-size:14px">
...
export class ClientMessagesComponent implements OnDestroy {
  ...
  async send() {
    ...
    try {
      await this.msgSvc.sendMessage({ companyId, companyName: this.selectedCompanyName(), clientId: uid, clientName: this.auth.displayName() || this.auth.currentUser()?.email || 'Cliente', senderRole: 'client', text });
    } catch (err) {
